...
//
// Returns a new rego instance with v0 as the default rego-version.
// To use a different rego-version, use the SetRegoVersion option,
// or use [github.com/open-policy-agent/opa/v1/rego.New] instead to default to v1.
//
// Deprecated: Use [github.com/open-policy-agent/opa/v1/rego.New] instead.
func New(options ...func(r *Rego)) *Rego {
	options = append(options, func(r *Rego) {
		if r.RegoVersion() == ast.RegoUndefined {
			SetRegoVersion(ast.RegoV0)(r)
		}
	})

	return v1.New(options...)
}
