...
func TestErrorAggregator(t *testing.T) {
	loc := &ast.Location{Text: []byte("example.rego"), Row: 1, Col: 1}
	...
	ea := newErrorAggregator(&errorList)

	ea.Add(err1, err2, err3)
	...
	if !err1.Is(&(errorList[0])) {
		t.Errorf("Expected %v, got %v", err1, errorList[0])
	}
	if !err3.Is(&(errorList[1])) {
		t.Errorf("Expected %v, got %v", err1, errorList[0])
	}
}
