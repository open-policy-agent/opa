...
-----END EC PRIVATE KEY-----`
	...
	t.Run("TestParseECPrivateKey", func(t *testing.T) {
		parsed, err := getRSAPrivateKeyFromString(keyPemEC)
		if err != nil {
			t.Fatalf("failed to parse PEM cert: %v", err)
		}
		if _, err := jwk.New(parsed); err != nil {
...
MHcCAQEEINvr5/ubwMwNmwAvcg7j6PXVJU9ArXjDGSA98CuGil4yoAoGCCqGSM49
AwEHoUQDQgAEFhb1oUsRLKWjyIir6XXEA9JunJrr5iJ5GBaWP0G67Ei/53pN3kKT
3unMgVLFOUTBJIoIpBQWHR6E9qml3rIS1g==
-----END EC PRIVATE KEY-----
`)
	...
	t.Run("TestParseRSAPrivateKey", func(t *testing.T) {
		...
		_, parseErr := parsePrivateKey(parsed)
		if parseErr != nil {
	...
	t.Run("TestParseECPrivate", func(t *testing.T) {

		parsed, err := getRSAPrivateKeyFromPEM(keyPemEC)
		if err != nil {
			t.Fatalf("failed to parse PEM cert: %v", err)
		}

		theKey, parseErr := parsePrivateKey(parsed)
		if parseErr != nil {
			t.Fatalf("we expect this to return no errors, but it returned: %v", parseErr)
		}
...
=======
		theJson, err := json.Marshal(theKey)
>>>>>>> 60f70a55c (init new builtin crypto.parse_private_key)
=======
		theJSON, err := json.Marshal(theKey)
>>>>>>> 34cf111bd (var-naming: var theJson should be theJSON)
=======
		theJSON, err := json.Marshal(theKey)
...
>>>>>>> ab3997c09 (var-naming: var theJson should be theJSON)
	...
	t.Run("TestParseECPrivateb64", func(t *testing.T) {
		b64 := base64.StdEncoding.EncodeToString(keyPemEC)
