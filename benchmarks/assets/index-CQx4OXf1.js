...
const DuckDBClient = () => __vitePreload(() => import("./duckdb-Bi5r1Cd9.js"), true ? [] : void 0).then((_2) => _2.DuckDBClient);
const echarts = () => __vitePreload(() => import("https://cdn.jsdelivr.net/npm/echarts/+esm"), true ? [] : void 0);
...
const L = () => __vitePreload(() => import("./leaflet-6qAF3qIa.js"), true ? [] : void 0);
const mapboxgl = () => __vitePreload(() => import("./mapboxgl-C0i2HzjJ.js"), true ? [] : void 0).then((_2) => _2.default);
const md = () => __vitePreload(() => import("./md-DxFABnfi.js"), true ? [] : void 0).then((_2) => _2.md);
const mermaid = () => __vitePreload(() => import("./mermaid-CLS82-Jo.js"), true ? [] : void 0).then((_2) => _2.mermaid);
...
const ReactDOM = () => __vitePreload(() => import("https://cdn.jsdelivr.net/npm/react-dom/+esm"), true ? [] : void 0);
...
define(
  ...
  },
  {
    ...
    body: (FileAttachment2) => {
      return FileAttachment2(new URL("/opa/benchmarks/assets/node-2sqbms3s85uuh6dw-BR2LXxiW.json", import.meta.url).href).json();
    },
...
define(
  ...
  },
  {
    id: 3,
    body: (FileAttachment2) => {
      return FileAttachment2(new URL("/opa/benchmarks/assets/node-2g3692rdkgyjct7s-1e9N8_Fr.json", import.meta.url).href).json();
    },
    inputs: ["FileAttachment"],
    outputs: [],
    output: "commits",
...
define(
  ...
  },
  {
    id: 4,
    body: (FileAttachment2) => {
      return FileAttachment2(new URL("/opa/benchmarks/assets/node-4yu6e2vd2b09w8ez-DaS1bqxj.json", import.meta.url).href).json();
    },
    inputs: ["FileAttachment"],
    outputs: [],
    output: "tags",
...
define(
  ...
  {
    id: 5,
    body: (benchmarks, tags, commits, d32) => {
      ...
      const rows = flatten(benchmarks, tags).filter((d) => commits[d.commit]);
      const cvByBenchmark = /* @__PURE__ */ new Map();
      const grouped = d32.group(rows, (d) => `${d.pkg}\0${d.name}`);
      for (const [key, values] of grouped) {
        const nsValues = values.filter((d) => d.measure === "NsPerOp").map((d) => d.value);
        if (nsValues.length > 1) {
          const mean = d32.mean(nsValues);
          const std = Math.sqrt(d32.mean(nsValues.map((v) => (v - mean) ** 2)));
          cvByBenchmark.set(key, mean !== 0 ? std / Math.abs(mean) : 0);
        }
      }
      return { flatten, rows, cvByBenchmark, grouped };
    },
    inputs: ["benchmarks", "tags", "commits", "d3"],
    outputs: ["flatten", "rows", "cvByBenchmark", "grouped"],
    output: void 0,
    assets: void 0,
    autodisplay: false,
    autoview: void 0,
...
define(
  ...
  },
  {
    id: 7,
    body: (view, Inputs2) => {
      const hideNoisy = view(Inputs2.toggle({ label: "Hide noisy", value: true }));
      return { hideNoisy };
    },
    inputs: ["view", "Inputs"],
    outputs: ["hideNoisy"],
...
define(
  ...
  {
    id: 8,
    body: (d32, rows, hideNoisy, cvByBenchmark, htl2, view, Inputs2) => {
      ...
      const filteredBenchmarks = hideNoisy ? benchmarksWithRelDiffs.filter((d) => (cvByBenchmark.get(`${d.pkg}\0${d.name}`) || 0) <= 0.2) : benchmarksWithRelDiffs;
      ...
      numericKeys.forEach((key) => {
        ...
        filteredBenchmarks.forEach((row) => {
          const value = row[key];
  ...
  ">${value.toFixed(precision)}</div>`;
      ...
      const search = view(Inputs2.search(filteredBenchmarks, {
        label: "Benchmarks",
        filter: columnFilter(["pkg", "name"]),
        autocomplete: true
      }));
      return { relativeDifferencesByMeasure, benchmarksWithRelDiffs, filteredBenchmarks, numericKeys, minMaxByMeasureKeyLog2, scalesByMeasureKey, colorScaleBackground, termFilter, escapeRegExp, columnFilter, search };
    },
    inputs: ["d3", "rows", "hideNoisy", "cvByBenchmark", "htl", "view", "Inputs"],
    outputs: ["relativeDifferencesByMeasure", "benchmarksWithRelDiffs", "filteredBenchmarks", "numericKeys", "minMaxByMeasureKeyLog2", "scalesByMeasureKey", "colorScaleBackground", "termFilter", "escapeRegExp", "columnFilter", "search"],
    output: void 0,
    assets: void 0,
    autodisplay: false,
    autoview: void 0,
...
define(
  ...
  {
    id: 9,
    body: (view, Inputs2, search, filteredBenchmarks, colorScaleBackground) => {
      const selectedRow = view(Inputs2.table(search.length > 0 ? search : filteredBenchmarks, {
        multiple: false,
        value: search.length > 0 ? search[0] : filteredBenchmarks[0],
        width: {
          pkg: 200,
          name: 300
        },
    ...
    inputs: ["view", "Inputs", "search", "filteredBenchmarks", "colorScaleBackground"],
    outputs: ["selectedRow"],
    output: void 0,
    assets: void 0,
    autodisplay: false,
...
define(
  {
    root: document.getElementById(`cell-13`),
    expanded: [],
    variables: []
  },
  {
