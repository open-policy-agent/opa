...
func (c *Config) validateAndInjectDefaults(services []string, plugins []string) error {
	...
	if c.Resource == "" {
		c.Resource = defaultResourcePath
	} else {
		if _, err := url.Parse(c.Resource); err != nil {
			return fmt.Errorf("invalid resource path %q: %v", c.Resource, err)
		}
	}
...
func (p *Plugin) oneShot(ctx context.Context) (ok bool, err error) {
	...
	for bs := oldBuffer.Pop(); bs != nil; bs = oldBuffer.Pop() {
		if err == nil {
			err = uploadChunk(ctx, p.manager.Client(p.config.Service), p.config.Resource, p.config.PartitionName, bs)
		}
		if err != nil {
			if p.limiter != nil {
...
func uploadChunk(ctx context.Context, client rest.Client, resourcePath string, partitionName string, data []byte) error {

	logPath := fmt.Sprintf("%v", resourcePath)
	if partitionName != "" {
		logPath = fmt.Sprintf("/logs/%v", partitionName)
	}
