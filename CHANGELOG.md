...
- A reference looking a subject up in a collection in base data
  (`data.groups.admins.members[input.subject]`) is indexed by asking that collection,
  where a ruleset of one rule per group used to leave every rule a candidate.
