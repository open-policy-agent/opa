...
func (rt *Runtime) Serve(ctx context.Context) error {
	...
	if lp := logs.Lookup(rt.Manager); lp != nil {
		rt.server = rt.server.WithNDBCacheEnabled(rt.Params.NDBCacheEnabled || rt.Manager.NDBuiltinCacheEnabled())
	}
...
func verifyAuthorizationPolicySchema(m *plugins.Manager) error {
	authorizationDecisionRef, err := ref.ParseDataPath(m.DefaultAuthorizationDecision())
	if err != nil {
		return err
	}
