...
//
// A committed want_ast is a golden file, for the reasons the corpus README gives. What this
// test buys on top of reviewing the regeneration diff is failing when someone edits the
// parser without running `make generate`.
...
func assertWantAST(t *testing.T, tc parsercases.TestCase, what string, node any) {
	...
	if module, ok := node.(*Module); ok {
		// Dropped to match what the generator writes; conformance.MarshalOptions says why.
		module.Comments = nil
	}
