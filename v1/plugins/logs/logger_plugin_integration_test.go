...
func (p *testLoggerPlugin) Reconfigure(context.Context, any) {}
...
func TestDecisionLogsWithLoggerPlugin(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if !bytes.Contains([]byte(logged), []byte("test-decision-123")) {
			t.Errorf("expected decision_id in log output, got: %s", logged)
		}
		if !bytes.Contains([]byte(logged), []byte("data/test/allow")) {
			t.Errorf("expected path in log output, got: %s", logged)
		}
		if !bytes.Contains([]byte(logged), []byte("openpolicyagent.org/decision_logs")) {
...
func TestDecisionLogsWithBothConsoleAndLoggerPlugin(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if !bytes.Contains([]byte(pluginLogged), []byte("test-decision-456")) {
			t.Errorf("expected decision_id in logger plugin output")
		}
		if !bytes.Contains([]byte(consoleLogged), []byte("test-decision-456")) {
