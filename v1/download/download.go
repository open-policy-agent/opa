...
func (d *Downloader) loop(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			...
			delay = util.DefaultBackoff(float64(minRetryDelay), float64(*d.config.Polling.MaxDelaySeconds)*float64(time.Second), retry)
			...
			min := float64(*d.config.Polling.MinDelaySeconds) * float64(time.Second)
			max := float64(*d.config.Polling.MaxDelaySeconds) * float64(time.Second)
