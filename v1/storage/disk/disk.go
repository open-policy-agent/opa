...
func (db *Store) Truncate(ctx context.Context, txn storage.Transaction, params storage.TransactionParams, it storage.Iterator) error {
	...
	for {
		...
		if update.IsPolicy {
			err = underlyingTxn.UpsertPolicy(ctx, strings.Join(update.Path, "/"), update.Value)
			if err != nil {
				...
				if err = underlyingTxn.UpsertPolicy(ctx, strings.Join(update.Path, "/"), update.Value); err != nil {
					return wrapError(err)
				}
			}
		} else {
