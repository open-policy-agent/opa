...
ci-binary-smoke-test-%:
	...
	"$(RELEASE_DIR)/$(BINARY)" eval --format pretty --bundle test/cli/smoke/golden-bundle.tar.gz --input test/cli/smoke/input.json data.test.result --fail
	"$(RELEASE_DIR)/$(BINARY)" exec --bundle test/cli/smoke/golden-bundle.tar.gz --decision test/result test/cli/smoke/input.json
	"$(RELEASE_DIR)/$(BINARY)" build --output o0.tar.gz test/cli/smoke/test.rego
	"$(RELEASE_DIR)/$(BINARY)" eval --format pretty --bundle o0.tar.gz --input test/cli/smoke/input.json data.test.result --fail
	"$(RELEASE_DIR)/$(BINARY)" build --optimize 1 --output o1.tar.gz test/cli/smoke/test.rego
	"$(RELEASE_DIR)/$(BINARY)" eval --format pretty --bundle o1.tar.gz --input test/cli/smoke/input.json data.test.result --fail
	"$(RELEASE_DIR)/$(BINARY)" build --optimize 2 --output o2.tar.gz test/cli/smoke/test.rego
	"$(RELEASE_DIR)/$(BINARY)" eval --format pretty --bundle o2.tar.gz --input test/cli/smoke/input.json data.test.result --fail
