...
| <span class="opa-keep-it-together">`output := jsonpatch.apply(patch, target)`</span> | `output` is the result of applying `patch`, a [JSON Patch Document](https://tools.ietf.org/html/rfc6902#section-3), on the `target`. For example: `jsonpatch.apply([{"op": "remove", "path": "/a"}, {"op": "add", "path": "/c", "value": [4, 7]}], {"a": 1, "b": 2})` will result in `{"b": 2, "c": [4, 7]}`. |
