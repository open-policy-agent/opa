...
| <span class="opa-keep-it-together">``glob.match(pattern, value)``</span> | 2 | true if the ``value`` matches the glob ``pattern``. See [this glob library](https://github.com/gobwas/glob) for usage |
