...
| `[_].evaluated_rules`              | `array[string]` | List of annotation `id` values for rules that were successfully evaluated. Only present when `decision_logs.evaluated_rules` is enabled in the configuration. |
