...
strings, but unquoted YAML values like `public-egress: true` parse as booleans
in JSON. Use `is_string` to validate that all annotation values are strings.
For full document schemas, see
