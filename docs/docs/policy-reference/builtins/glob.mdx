...
`glob.match` matches strings against glob patterns with optional delimiters. The function returns `true` if the string matches the pattern and `false` otherwise.
...
<PlaygroundExample dir={require.context("../_examples/glob/multiple_delimiters")} />
