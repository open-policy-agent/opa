...
| Metric | Description |
| ------ | ----------- |
| `counter_rego_builtin_regex_interquery_value_cache_hits` | Number of regex cache hits for compiled patterns. Only appears when [inter-query caching is enabled](/docs/configuration/#caching) in OPA's configuration and the same regex pattern is compiled and reused multiple times |
