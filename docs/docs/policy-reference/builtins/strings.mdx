...
`split` returns a list of substrings separated by a delimiter. Path and name
parsing commonly use the second or third segment after a fixed prefix.

<PlaygroundExample dir={require.context('../_examples/strings/split/team-from-path')} />

### `lower`

`lower` converts a string to lowercase. Pair it with a list of allowed values when callers
send mixed-case roles, tenants, or usernames.
