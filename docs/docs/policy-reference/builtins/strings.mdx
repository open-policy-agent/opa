...
`endswith` reports whether a string ends with a given suffix. Use this function when the match must be at the end of the value (file extensions, email domains, path suffixes etc.).
...
another. A common policy use is redacting secrets or other sensitive values
before a message is returned to a caller.
