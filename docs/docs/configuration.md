...
| `decision_logs.evaluated_rules`                    | `boolean` | No (default: `false`)            | Include the annotation `id` of each successfully evaluated rule in decision log events. Requires rules to have `# METADATA` blocks with an `id` field. When any module contains an `id` annotation, metadata parsing is enabled automatically.          |
